...
const ChatInterface = () => {
  ...
  // Detect auth state so we only persist for signed-in users
  useEffect(() => {
    let cancelled = false;
  ...
  // Clear guest session storage when closing the tab so it doesn't linger across visits
  useEffect(() => {
  ...
  // Load persisted chat state once auth check is done
  useEffect(() => {
    if (!authChecked || !storageInfo) return;
    setHydrated(false);
    const storage =
      storageInfo.driver === 'session'
  ...
  // Persist chat state (localStorage for auth, sessionStorage for guest)
  useEffect(() => {
  ...
  const mapToUiProduct = (p: ProductMin): Product => {
    const priceStr =
      typeof p.Price === 'number' ? `${p.Price.toLocaleString('vi-VN')} ₫` : String((p as any).Price ?? '');
