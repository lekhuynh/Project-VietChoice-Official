        ...
        <div className="p-6">
                          ...
                          {item.Products.slice(0, 6).map((p) => (
                            <a key={p.Product_ID} href={`/product/${p.Product_ID}`} className="flex items-center gap-2 p-2 bg-white border rounded hover:shadow-sm">
                              ...
                              <div>
                                ...
                                <div className="text-xs text-gray-500">{typeof p.Price === 'number' ? p.Price.toLocaleString('vi-VN') + ' đ' : ''}</div>
                              </div>
                            </a>
                          ))}
                        </div>
          ...
          {activeTab === 'conversations' && !selectedConversation && (
            <div>
              ...
              <p className="text-gray-500">Hiện chưa có API trò chuyện trong backend để đồng bộ. Phần này sẽ được bổ sung sau.</p>
            </div>
          )}
